...
function genericFailure(res) {
  return res.status(401).json({
    error: 'Code ou challenge invalide.',
  });
}
...
async function getMfaStatus(req, res, next) {
  try {
    const m =
      await mfaRepository.findUserMfaByUserId(
        req.auth.userId
      );
...
async function setupMfa(req, res, next) {
  try {
    ...
    const encrypted =
      mfaService.encryptTotpSecret(secret);

    await mfaRepository.upsertPendingMfa({
    ...
    return res.json({
      ...
      otpauthUri,
      qrCodeDataUrl:
        await mfaService.generateQrCodeDataUrl(
          otpauthUri
        ),
    });
  } catch (e) {
...
async function confirmMfa(req, res, next) {
  ...
  try {
    ...
    if (
      !m ||
      m.enabled ||
      !mfaService.verifyTotpCode(
        mfaService.decryptTotpSecret(
          m.totp_secret_ciphertext,
          m.totp_secret_iv,
          m.totp_secret_auth_tag
        ),
        req.body.code
      )
    ) {
...
async function completeChallenge(
  ...
  try {
    ...
    const hash =
      mfaService.hashChallengeToken(
        req.body.challenge
      );

    const c =
      await mfaRepository
        .findActiveChallengeForUpdate({
          challengeHash: hash,
          purpose: 'login',
          client,
        });
    ...
    if (recovery) {
      valid = Boolean(
        await mfaRepository.consumeRecoveryCode({
          userId: c.user_id,
          codeHash:
            mfaService.hashRecoveryCode(
...
async function disableMfa(req, res, next) {
  ...
  try {
    ...
    if (!valid) {
      ...
      return res.status(401).json({
        error:
          'Mot de passe ou code MFA invalide.',
      });
    }

    await mfaRepository.deleteUserMfa(
      req.auth.userId,
      client
    );

    await mfaRepository.deleteRecoveryCodes(
      req.auth.userId,
      client
    );
    ...
    return res.json({
      ...
      message:
        'MFA désactivée. Reconnexion requise.',
    });
  } catch (e) {
    await client.query('ROLLBACK');
    return next(e);
